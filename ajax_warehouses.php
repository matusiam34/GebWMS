...
*/
				...
				if ($action_code == 20)
				{
					...
					{
						$sql	.=	'	AND wh_pkey = ' . $user_warehouse_uid;
					}
					else
					{

						//	Add All warehouses when it is someone who has the power to see all!
						$data_results[] =
						[
							'wh_pkey'		=> '0',
							'wh_code'		=> $mylang['all'],
				...
				if ($stmt = $db->prepare($sql))
				{
					...
					if ($action_code == 0)
					{
						...
						foreach ($data_results as $item)
						{
							...
							$html_results	.=	'<tr data-id="' . leave_numbers_only($item['wh_pkey']) . '" class="' . $color_code . '" >';
								$html_results	.=	'<td>' . trim($item['wh_code']) . '</td>';
							$html_results	.=	'</tr>';
