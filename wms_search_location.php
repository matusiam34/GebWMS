...
<?php
	...
	try
	{
		...
		//	The system does support multi warehouses and multi companies so that all needs to be
...
//							$location_details	.=	'<td style="background-color: ' . $backclrB . '; ' . $loc_pickface_style . '">' . trim($row['loc_code']) . ' (' . $loc_status_code_str . ')</td>';
						...
						//	Only show any notes if the location has been assigned any!
						$location_note	=	trim($row['loc_note']);
						
						if (strlen($location_note > 0))
						{

							$location_details	.=	'<tr>';
								$location_details	.=	'<td style="width:40%; background-color: ' . $backclrA . '; font-weight: bold;">' . $mylang['note'] . ':</td>';
								$location_details	.=	'<td style="background-color: ' . $backclrB . ';">' . $location_note . '</td>';
							$location_details	.=	'</tr>';
				...
				// Calculate amount of CASES if stk_unit indicates it to be a CASE (id = 3)
				...
				{
					//	If I was to store everything as EACH == this would be the way.
