...
//	NOTE: save (action 3) needs to run a SELECT on the uom code to see if there is one already with that name before updating!
...
/*
	...
	0	:	Get all UOMs (HTML, for a table).
	1	:	Get one UOM info (array).
	2	:	Add UOM!
	3	:	Update UOM details!

 
	20	:	Get all UOMs that are active. Output: array!
...
*/
				...
				//	When things go sideways with the 0.0 not showing at all maybe try this?!
				//	*, CAST(uom_conv_factor AS REAL) AS uom_conv_factor
				...
				{
					$sql	.=	'	WHERE uom_owner = :suom_owner';
				}
				elseif ($action_code == 11)	//	Active UOMs ONLY!
				{
					$sql	.=	'	WHERE uom_disabled = 0 AND uom_owner = :suom_owner';
				}
				...
				$sql	.=	' ORDER BY uom_disabled ASC, uom_code';
				...
				if ($stmt = $db->prepare($sql))
				{
					...
					if ($action_code == 10)
					{
						...
						foreach ($data_results as $item)
						{
							...
							$disabled	=	leave_numbers_only($item['uom_disabled']);
							
							if ($disabled == 1)	{		$color_code		=	'red_class';	}
							
							$html_results	.=	'<tr data-id="' . leave_numbers_only($item['uom_pkey']) . '" class="' . $color_code . '" >';
								$html_results	.=	'<td>' . trim($item['uom_code']) . '</td>';
							$html_results	.=	'</tr>';
						}
